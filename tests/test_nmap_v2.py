...
@pytest.fixture
def apt_platform():
    """Run an installer-flow test in its explicitly supported APT context."""
    with patch.object(profile_registry, "detect_os_key", return_value="linux-apt"):
        yield
...
@patch("ternexar.ui.ui.render_install_preview")
def test_nmap_install_preview_unsupported_linux(mock_render):
    """Verify generic Linux does not fall back to the APT profile."""
    with patch.object(profile_registry, "detect_os_key", return_value="linux-generic"):
        handle_install_preview("nmap")

    data = mock_render.call_args.args[0]
    assert data["detected_os"] == "linux-generic"
    assert data["status"] == ProfileStatus.NEEDS_VERIFICATION.value
    assert data["steps"] == []
